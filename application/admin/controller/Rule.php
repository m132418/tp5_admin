...
//*****************权限-用户组*****************
    ...
    public function rule_group(){
        if(request()->isPost()){
          ...
          $data['rule_ids'] =  array_unique($data['rule_ids']) ;
...
//             $data['rules']=implode(',', $data['rule_ids']);
            ...
            $ag =  AuthGroup::get($data['id']) ;
            $ag->rules= implode(',', $data['rule_ids']);
            $result=$ag->save();
            if ($result) {
...
//******************用户-用户组*******************
    ...
    public function add_admin(){
        if(request()->isPost()){
            $data=input('post.');
            $result=Db::name ('Users')->addData($data);
            if($result){
                ...
                // 操作成功
             return   $this->success('添加成功',url('Admin/Rule/admin_user_list'));
            }else{
